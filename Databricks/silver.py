...
#defining the schema while fetching rides_raw json field
...
#Empty staging table creation for performing append flow using sdp.
dp.create_streaming_table("stg_rides")

#adding bulk data or initial data load to append flow
@dp.append_flow(
    target = 'stg_rides'
)
def rides_bulk():
...
#adding streaming table load to append flow
@dp.append_flow(
    target = 'stg_rides'
)
def rides_stream():
