...
def dim_driver_view():
    df = spark.readStream.table("silver_obt")
    df = df.select("driver_id", "driver_name", "driver_rating", "driver_license")
...
def fact_view():
    df = spark.readStream.table("silver_obt")
    df = df.select("ride_id", "pickup_city_id", "payment_method_id", "vehicle_id", "driver_id", "passenger_id", "distance_miles", "duration_minutes", "base_fare", "distance_fare", "time_fare", "surge_multiplier", "total_fare", "tip_amount", "rating", "base_rate", "per_mile", "per_minute")
    df = df.dropDuplicates(subset=['ride_id'])
    return df
...
dp.create_auto_cdc_flow(
  target = "fact",
  source = "fact_view",
  keys = ["ride_id", "pickup_city_id", "payment_method_id", "vehicle_id", "driver_id", "passenger_id"],
  sequence_by = "ride_id",
  stored_as_scd_type = 1
)
