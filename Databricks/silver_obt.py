...
df_parsed = df.withColumn("Parsed_rides", from_json(col("rides"), rides_schema)).select(col("Parsed_rides.*"))
display(df_parsed)
...
# MAGIC %md ####Jinja Template for silver obt
...
templates = Template(jinja_str)
rendered_template = templates.render(jinja_config=jinja_config)
...
# mvt.*,
# mvm.*
# from uber.bronze.stg_rides stg_rides
# left join
# uber.bronze.map_vehicle_types mvt
# on stg_rides.vehicle_type_id =  mvt.vehicle_type_id
# left join 
# uber.bronze.map_vehicle_makes mvm
# on stg_rides.vehicle_make_id = mvm.vehicle_make_id
